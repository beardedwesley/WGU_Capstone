...
public class DerbyDBDriver {
    ...
    private static void genTables(Connection dbConn) throws SQLException, URISyntaxException {
        String file = "/main/resources/createTables.txt";
        executeScript(file);

        //Comment out next line to prevent testing data placed in database
        executeScript("/main/resources/fillTestData.txt");
    }

    /* Helper functions */
    private static void executeScript(String file) {
        ScriptHelper scriptHelper = new ScriptHelper(file);
        for (String line : scriptHelper.getLines()) {
            try {
